...
assert.match(bridge,/processing\s*=\s*'pending'/);
assert.match(bridge,/ok\s*:\s*true[\s\S]*processing\s*:\s*processing/,'pós-processamento falho não pode invalidar persistência');
assert.match(orch,/scoreScreeningV3_/);
assert.match(orch,/enqueueScreeningPipeline_/);
assert.match(orch,/payload\.identity/);
assert.ok(!orch.includes('payload.answers'),'orquestrador não pode voltar ao contrato por título');
assert.match(orch,/SUICIDE_ITEM_ENDORSED/);
assert.match(report,/ALERTA CLÍNICO/);
assert.match(score,/SUICIDAL_IDEATION_PRESENT/);
console.log('SCREENING_POSTPROCESSING_V31_STATIC_PASS');
